...
from .v3_graph_api import router as v3_graph_router

app = FastAPI(title="Recovery Intelligence API", version="0.8.0")
...
app.include_router(v3_graph_router)
...
class CaseResponse(BaseModel):
    ...
    id: UUID; case_number: str; borrower_name: str; status: str; priority: str
    known_phone: str | None; known_email: str | None; known_address: str | None
class InvestigationRequest(BaseModel):
    case_id: str; depth: str = "STANDARD"; fields: list[str] = Field(default_factory=list); urls: list[str] = Field(default_factory=list)
class SourceCreate(BaseModel):
    source_id: str = Field(min_length=2, max_length=120); source_name: str = Field(min_length=2, max_length=200)
    source_type: str = Field(min_length=2, max_length=60); base_url: str | None = None
    allowed_domains: list[str] = Field(default_factory=list); allowed_fields: list[str] = Field(default_factory=list)
    rate_limit_per_minute: int = Field(default=30, ge=1, le=10000); enabled: bool = True
class SourceResponse(BaseModel):
    model_config = ConfigDict(from_attributes=True)
    id: UUID; source_id: str; source_name: str; source_type: str; base_url: str | None
    allowed_domains: list; allowed_fields: list; rate_limit_per_minute: int; enabled: bool
class EvidenceResponse(BaseModel):
    model_config = ConfigDict(from_attributes=True)
    id: UUID; source_name: str; source_type: str; source_reference: str | None; source_url: str | None
    entity_type: str | None; field_name: str | None; normalized_value: str | None; verification_status: str
    confidence: float | None; content_hash: str | None; extraction_method: str | None; retrieved_at: datetime
class ReviewResponse(BaseModel):
    model_config = ConfigDict(from_attributes=True)
    id: UUID; review_type: str; status: str; reason: str; payload: dict; resolution: str | None; created_at: datetime
...
async def register(payload: RegisterRequest, db: AsyncSession = Depends(get_db)) -> TokenResponse:
    ...
    org = Organization(name=payload.organization_name, slug=payload.organization_slug); db.add(org); await db.flush()
    ...
    db.add(user); await db.flush(); db.add(AuditLog(organization_id=org.id, user_id=user.id, action="REGISTER")); await db.commit()
...
async def login(payload: LoginRequest, db: AsyncSession = Depends(get_db)) -> TokenResponse:
    ...
    db.add(AuditLog(organization_id=user.organization_id, user_id=user.id, action="LOGIN")); await db.commit()
...
async def list_cases(user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)) -> list[CaseResponse]:
    result = await db.scalars(select(Case).where(Case.organization_id == user.organization_id).order_by(Case.created_at.desc())); return list(result.all())
...
async def create_case(payload: CaseCreate, user: User = Depends(require_roles("organization_admin", "recovery_manager")), db: AsyncSession = Depends(get_db)) -> CaseResponse:
    if await db.scalar(select(Case).where(Case.organization_id == user.organization_id, Case.case_number == payload.case_number)):
    ...
    db.add(case); await db.flush(); db.add(AuditLog(organization_id=user.organization_id, user_id=user.id, case_id=case.id, action="CASE_CREATE")); await db.commit(); await db.refresh(case)
...
async def list_sources(user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)) -> list[SourceResponse]:
    result = await db.scalars(select(SourceRegistry).where((SourceRegistry.organization_id == user.organization_id) | (SourceRegistry.organization_id.is_(None)), SourceRegistry.enabled.is_(True)).order_by(SourceRegistry.source_name)); return list(result.all())
...
async def create_source(payload: SourceCreate, user: User = Depends(require_roles("organization_admin")), db: AsyncSession = Depends(get_db)) -> SourceResponse:
    if await db.scalar(select(SourceRegistry).where(SourceRegistry.organization_id == user.organization_id, SourceRegistry.source_id == payload.source_id)):
        raise HTTPException(status_code=409, detail="Source already exists for this organization")
    source = SourceRegistry(organization_id=user.organization_id, **payload.model_dump()); db.add(source); await db.flush(); db.add(AuditLog(organization_id=user.organization_id, user_id=user.id, action="SOURCE_REGISTER", details=f"source_id={source.source_id}")); await db.commit(); await db.refresh(source)
...
async def create_investigation(request: InvestigationRequest, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)) -> dict:
    try: case_id = UUID(request.case_id)
    except ValueError as exc: raise HTTPException(status_code=400, detail="Invalid case_id") from exc
    case = await db.scalar(select(Case).where(Case.id == case_id, Case.organization_id == user.organization_id))
    if not case: raise HTTPException(status_code=404, detail="Case not found")
    investigation = Investigation(organization_id=user.organization_id, case_id=case.id, requested_by=user.id, depth=request.depth, status="QUEUED"); db.add(investigation); await db.flush()
    for url in request.urls: db.add(ScraperJob(organization_id=user.organization_id, investigation_id=investigation.id, job_type="PUBLIC_FETCH", status="QUEUED", requested_url=url, requested_fields=request.fields))
    db.add(AuditLog(organization_id=user.organization_id, user_id=user.id, case_id=case.id, action="INVESTIGATION_QUEUED", details=f"investigation={investigation.id};depth={request.depth};jobs={len(request.urls)}")); await db.commit()
...
async def investigation_progress(investigation_id: str, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)) -> dict:
    try: inv_id = UUID(investigation_id)
    except ValueError as exc: raise HTTPException(status_code=400, detail="Invalid investigation_id") from exc
    ...
    jobs = list((await db.scalars(select(ScraperJob).where(ScraperJob.investigation_id == investigation.id))).all()); counts: dict[str,int] = {}
    for job in jobs: counts[job.status] = counts.get(job.status, 0) + 1
    total=len(jobs); completed=counts.get("COMPLETED",0); failed=counts.get("FAILED",0)
    progress=round((completed+failed)/total*100,1) if total else (100 if investigation.status=="COMPLETED" else 0)
...
async def case_evidence(case_id: str, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)) -> list[EvidenceResponse]:
    try: cid = UUID(case_id)
    except ValueError as exc: raise HTTPException(status_code=400, detail="Invalid case_id") from exc
    case = await db.scalar(select(Case).where(Case.id == cid, Case.organization_id == user.organization_id))
    if not case: raise HTTPException(status_code=404, detail="Case not found")
    rows = await db.scalars(select(Evidence).where(Evidence.organization_id == user.organization_id, Evidence.case_id == cid).order_by(Evidence.retrieved_at.desc())); return list(rows.all())
...
async def review_queue(case_id: str, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)) -> list[ReviewResponse]:
    ...
    case = await db.scalar(select(Case).where(Case.id == cid, Case.organization_id == user.organization_id))
    if not case: raise HTTPException(status_code=404, detail="Case not found")
    rows = await db.scalars(select(ReviewQueue).where(ReviewQueue.organization_id == user.organization_id, ReviewQueue.case_id == cid, ReviewQueue.status == "PENDING").order_by(ReviewQueue.created_at.desc())); return list(rows.all())
...
async def review_decision(review_id: str, decision: ReviewDecision, user: User = Depends(require_roles("organization_admin", "recovery_manager", "auditor")), db: AsyncSession = Depends(get_db)) -> ReviewResponse:
    try: rid = UUID(review_id)
    except ValueError as exc: raise HTTPException(status_code=400, detail="Invalid review_id") from exc
    review = await db.scalar(select(ReviewQueue).where(ReviewQueue.id == rid, ReviewQueue.organization_id == user.organization_id))
    if not review: raise HTTPException(status_code=404, detail="Review item not found")
    review.status = decision.status; review.resolution = decision.resolution; review.resolved_at = datetime.now(timezone.utc); db.add(AuditLog(organization_id=user.organization_id, user_id=user.id, case_id=review.case_id, action="REVIEW_DECISION", details=f"review={review.id};status={decision.status}")); await db.commit(); await db.refresh(review)
